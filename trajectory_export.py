...
def render_views(source_path: str, gs_source : str, exp_folder: str, pipeline : PipelineParams, scene_key, output_path):
    ...
    fov_deg = train_views[0].FoVx / np.pi * 180.0
    render_height = 512
    render_width = 512
    seconds = 10
    camera_path_dict = {
    ...
    camera_path_dict["camera_path"] = camera_path_list
    
    camera_json = Path(source_path) / "camera_path.json"
    with open(camera_json, "w") as fp:
        json.dump(camera_path_dict, fp, indent=2)
    print(f"Saved camera path to {camera_json}")
